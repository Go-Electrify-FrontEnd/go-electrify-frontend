...
export async function uploadDocument(prev: unknown, formData: FormData) {
  try {
    ...
    try {
      parsedText = await parseDocument(buffer, file.type);
    } catch (error) {
      return {
        success: false,
    ...
    // Prepare chunks for Pinecone (embeddings will be generated by Pinecone)
    const textChunks = chunks.map((content) => ({ content }));

    // Upsert to Pinecone (Pinecone will generate embeddings using llama-text-embed-v2)
